...
<body>
    ...
    <div class="container-fluid main-container">
        ...
        <div class="fluid-row" id="header">
            ...
            <h1 class="title toc-ignore">Misallocation in brasilian labor market</h1>
            <h4 class="date">2020-07-22</h4>

        </div>


        <p>In this presentation we employ the modified Hsieh’s model to verify if there are misallocation in brasilian labor market. First I will import libraries.</p>
...
import pandas as pd</code></pre>
        <p>Now, I need define the parameters of model.</p>
...
def par():
    ...
    phi = np.array([0.138, 0.174, 0.136, 0.1, 0.051, 0.084, 0.168]).reshape(i, 1)
        ...
        <p>So I will define my initial guess of <span class="math inline">\(\tau^w\)</span>, <span class="math inline">\(\tau^h\)</span>, and <span class="math inline">\(w\)</span>.</p>
<pre class="python"><code>
def taus2():
    par()
...
x1 = taus2()</code></pre>
        <p>The time spent in school is given by:</p>
        <p><span class="math display">\[s^* = \left(1 + \frac{1-\eta}{\beta \phi}\right)^{-1}\]</span></p>
<pre class="python"><code>def sf( ):
    global s
    s = np.power( (1+ np.divide( (1-eta), ( np.multiply(beta, phi) ) ) ), -1 )
    s = s.reshape(i, 1)
    return s</code></pre>
        <p>The fraction of teachers, <span class="math inline">\(p_{tr}\)</span>, can be written as:</p>
        <p>
...
<pre class="python"><code>def p_trf(x1):    
    s = sf( )    
    ...
    return p_tr
        ...
        <p>The teacher’s human capital is given by:</p>
        <p>
            <span class="math display">
                \[
        ...
        <p>Where <span class="math inline">\(\kappa = 1/(1 - \eta)\)</span>, <span class="math inline">\(\pi = 1-(1-\alpha)\varphi\kappa\)</span>, <span class="math inline">\(\nu = 1+ \alpha\varphi\kappa -\theta\kappa\)</span>, and <span class="math inline">\(\sigma = \frac{\eta \kappa}{\pi}\)</span>.</p>
<pre class="python"><code>  
def H_trf(x1):    
    p_tr = p_trf(x1)  
    ...
    return H_tr
        ...
        <p>
            <span class="math display">
                ...
                \end{equation*}\]
            </span> where <span class="math inline">\(\psi = \eta^\eta(1-\eta)^{1-\eta}\)</span>. We can interpret <span class="math inline">\(\tilde{w}_{ir}\)</span> as a liquid reward for a person with mean ability from region <span class="math inline">\(r\)</span> and occupation <span class="math inline">\(i\)</span>. So, <span class="math inline">\(\tilde{w}_{ir}\)</span> is composed by wage per efficiency unit in the occupation <span class="math inline">\(w_{ir}\)</span> schooling, teacher’s human capital and frictions.
        </p>
<pre class="python"><code>def w_tilf(x1):
    H_tr = H_trf(x1) 
    p_tr = p_trf(x1)                 
    ...
    return w_til 
        ...
        <p>Where <span class="math inline">\(p_{ir}\)</span> is the fraction of people that work in occupation <span class="math inline">\(i\)</span> in region <span class="math inline">\(r\)</span></p>
<pre class="python"><code>
def p_irf(x1):    
    w_til = w_tilf(x1)
    ...
    return np.array(p_ir), np.array(w_r)
        ...
        <p>Let <span class="math inline">\(W_{ir}\)</span> be the gross average earnings in occupation <span class="math inline">\(i\)</span> in region <span class="math inline">\(r\)</span>. Then:</p>
        <p>
            <span class="math display">
        ...
        <p>Where <span class="math inline">\(\gamma= \Gamma(1-(\theta(1-\rho))^{-1}(1-\eta)^{-1})\)</span> is related to the mean of the Fréchet distribution for abilities.</p>
<pre class="python"><code>def Wf(x1):
    p_ir, w_r = p_irf(x1)         
    ...
    return W, p_ir
        ...
        <p>Now I need import PNAD data.</p>
<pre class="python"><code>def simul():
    global p_t, W_t
...
simul()
        ...
        <p>So, the problem consist in minimize equation below, using L-BFGS-B. Others algorithms can be used, for example genetic algorithm.</p>
        <p>
            <span class="math display">
...
<pre class="python"><code>def obj(x1):
...
<pre class="python"><code>%time obj(taus2())</code></pre>
...
def hessp(x, l):
    return np.zeros((3, i, r))
        ...
        <div id="optimization" class="section level3">
            <h3>Optimization</h3>
<pre class="python"><code>## Nelder Mead
